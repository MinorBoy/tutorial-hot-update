...
class Puppet {
    constructor(attr, rootNode) {
        this._battlePanel = rootNode.battlePanel
        this._battle = rootNode.battle
        ...
        if (index == ModelConsIndex.Left || index == ModelConsIndex.Right) {
            ...
            let mapPanel = this._battlePanel.getChildByName("mapPanel")
            this._bloodPanel = mapPanel.getChildByName("bloodPanel" + index)
            if (this._bloodPanel) {
                this._bloodPanel.setVisible(true)
            }
        }
        ...
        let info = ConfigDataManager.getInfoFindByOneKey(ConfigData.ModelGroConfig, "modelID", modelType)
        ...
        let hpTxt = this._infoPanel.getChildByName("hpTxt")
        hpTxt.setVisible(true) //初始化显示hp

        if (this._infoPanel != nil) {
    ...
    finalize() {
        ...
        this._rootNode = nil
        this._infoPanel = nil
        this._bloodPanel = nil
        this._battlePanel = nil
    }
    ...
    playBirthBuffEffect() {
        ...
        for (let key in this._birthBuffs) {
            ...
            let buffId = buff.id
            let info = ConfigDataManager.getConfigById(ConfigData.BuffConfig, buffId)
            if (info != nil && info.showID > 0) {
                //添加特效
    ...
    // 添加buff特效
    addBuffCCB(buffId) {
        let buffCfgData = ConfigDataManager.getConfigById(ConfigData.BuffConfig, buffId)
        if (buffCfgData == nil || buffCfgData.ccb == " ") {
            return
        }
        if (this._ccbMap[buffCfgData.ccb] == nil) {
            let pos = JSON.parse(buffCfgData.warDeviation)

            let ArmyCfgData = ConfigDataManager.getConfigById(ConfigData.ArmKindsConfig, this._modelType)
            let x = 0
            let y = 0
            if (ArmyCfgData != nil) {
    ...
    delBuffCCB(buffId) {
        ...
        this._ccbMap[buffCfgData.ccb] = nil
    }
    ...
    beHurt(bloods, num, callback, fCount, fsecond, delayDeath, delayHurt) {
        ...
        let endCall = (function () {
            ...
            if (this._curHp <= 0) {
                ...
                this._infoPanel.setVisible(true)  //TODO 这里有可能都死光了，不用再显示出来
            }
        ...
        let beDeathCallback = (function () {
            if (this._liveModelNum == 0) { //全死光了
                if (callback != nil) { //死光的时候是没有behurt回调的
    ...
    setModelName() {
        let nameTxt = this._infoPanel.getChildByName("nameTxt")
        let info = ConfigDataManager.getConfigById(
            ConfigData.ModelGroConfig, this._modelType)
        nameTxt.setString(info.modelName)
        // nameTxt.setString(info.modelName+this._modelType)  //测试

        //同时设置武将
        let heroId = this._heroId
        let heroImg = this._infoPanel.getChildByName("heroImg")
    ...
    setConsiNameVisible(visible) {
        let bgImg = this._infoPanel.getChildByName("bgImg")
        let nameTxt = this._infoPanel.getChildByName("nameTxt")
        let heroImg = this._infoPanel.getChildByName("heroImg")
        let starNum = this._infoPanel.getChildByName("starNum")
        let hpTxt = this._infoPanel.getChildByName("hpTxt")
        hpTxt.setVisible(!visible)
        nameTxt.setVisible(visible)

        let star = this._consiStar  //军师的星级，为0时不显示星级
        if (star == nil || star > 5) {  //star > 5 暂时先按0处理
            star = 0
        }
        ...
        if (star == 0) {  //没星时名字居中
            ...
            starNum.setletZOrder(25)
            TextureManager.updateImageView(heroImg, "images/newGui1/IconStarMini.png")
            heroImg.setScale(1)
            heroImg.setletZOrder(20)
            bgImg.setScaleX(1.5)

            let size1 = heroImg.getContentSize()
            let size2 = starNum.getContentSize()

            let par = nameTxt.getParent().getParent()
    ...
    hideInfoPanel() {
        if (this._rootNode == nil) {
            logger.info("~~~~~~~~~~Puppet.hideInfoPanel~~~~~~~index.%d~~~~~~~~~~~~~", this._index)
        }
        this._infoPanel.setVisible(false)
        this._rootNode.setVisible(false)
    }
    ...
    getZhenfaInfo(zhenfa, eye) {
        let info = ConfigDataManager.getInfoFindByThreeKey(ConfigData.ZhenfaConfig,
            "type", zhenfa, "eye", eye, "camp", this.camp)
        return info
    }
