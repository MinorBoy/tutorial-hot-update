...
class BattleProxy extends BasicProxy {
    ...
    //数据代理全部初始化前调用
    beforeInitSyncData() {       
        for (let key in GameConfig.battleType) {
            let v = GameConfig.battleType[key]
            if (v == 2) {
                let config = ConfigDataManager.getConfigData(ConfigData.AdventureConfig)
                for (let key in config) {
                    LocalDBManager.setValueForKey(GameConfig.isAutoBattle + v + config[key].type, "no")
                }
            } else {
    ...
    startBattleReq(data) {
        this.preUIAnimation()
        let isAuto = LocalDBManager.getValueForKey(GameConfig.isAutoBattle + data.type)
        if (data.type == 2) {
        ...
        if (data.type == 6) {
            ...
            sendData.id = data.id
            let str = LocalDBManager.getValueForKey(GameConfig.isAutoBattle + data.type)
    ...
    onTriggerNet50000Resp(data) {
        ...
        if (this._curBattleType == GameConfig.battleType.world_boss && this._hasWorldBossBattle == true) {
            ...
            if (data.type == 6) {
                ...
                //是否加入了同盟
                if (name != "") {
                    this.syncNetReq(AppEvent.NET_M22, AppEvent.NET_M22_C220007, { opt : 0 })
                }
            }
        }
    }
